...
func (uq *UserQuery) QueryCreatedSecrets() *SecretQuery {
	query := (&SecretClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withCreatedSecrets: uq.withCreatedSecrets.Clone(),
...
func (uq *UserQuery) WithCreatedSecrets(opts ...func(*SecretQuery)) *UserQuery {
	query := (&SecretClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withCreatedSecrets = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withCreatedSecrets; query != nil {
		if err := uq.loadCreatedSecrets(ctx, query, nodes,
			func(n *User) { n.Edges.CreatedSecrets = []*Secret{} },
			func(n *User, e *Secret) { n.Edges.CreatedSecrets = append(n.Edges.CreatedSecrets, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadCreatedSecrets(ctx context.Context, query *SecretQuery, nodes []*User, init func(*User), assign func(*User, *Secret)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(secret.FieldCreatedByUser)
	}
	query.Where(predicate.Secret(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.CreatedSecretsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.CreatedByUser
		if fk == nil {
			return fmt.Errorf(`foreign-key "created_by_user" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "created_by_user" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
